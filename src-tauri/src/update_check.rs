...
const LATEST_VERSION_URL: &str = "https://api.github.com/repos/Cowjiang/zerotier-toolkit/releases/latest";
#[derive(Deserialize)]
...
lazy_static! {
    static ref LATEST_VERSION_HOLDER: Arc<Mutex<LatestVersionResult>> =Arc::new(Mutex::new(LatestVersionResult {
...
async fn get_latest_version_from_github() -> Result<String, Error> {
    ...
    let response = client.get(LATEST_VERSION_URL)
        .header("User-Agent", "github/zerotier-toolkit")
        .send().await;
    if response.is_err() {
        return Err(Error::CheckFromRemoteError(response.err().unwrap().to_string()));
    }
    ...
    if !status_code.is_success() {
        ...
        error!("check update failed:{:?}",text_result.unwrap_or_else(|_| "empty body".to_string()));
        return Err(Error::CheckFromRemoteError("request failed,see details in log".to_string()));
    }
    let json_result = response.json::<LatestVersionResponseFromGit>().await;
    if json_result.is_err() {
        return Err(Error::CheckFromRemoteError(json_result.err().unwrap().to_string()));
    }
...
pub async fn get_latest_version_command(app_handle: AppHandle) -> String {
    ...
    if latest_version_holder.current_version.is_none() {
        latest_version_holder.current_version = Some(format!("v{}", app_handle.package_info().version.to_string()));
    }
    let latest_time = latest_version_holder.latest_time;
    if latest_time.is_none() || Local::now().signed_duration_since(latest_time.unwrap()).num_seconds() >= UPDATE_CHECK_INTERVAL {
        ...
        match latest_version {
            ...
            Err(err) => {
                return fail_message_json(format!("update check fail with:{}", err.to_string()).as_str())
            }
        }
    }
