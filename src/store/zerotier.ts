...
export const useZeroTierStore = create<ZeroTierState & ZeroTierAction>()(
  (set) => ({
    ...
    getServiceState: async () => {
      const { data: serviceState } = await invokeCommand(
        InvokeEvent.GET_SERVICE_STATE,
      )
    ...
    getServiceStartType: async () => {
      const { data: serviceStartType } = await invokeCommand(
        InvokeEvent.GET_SERVICE_START_TYPE,
      )
    ...
    setServiceStartType: async (startType: ServiceStartType) => {
      const { success } = await invokeCommand(
        InvokeEvent.SET_SERVICE_START_TYPE,
        { startType },
      )
    ...
    getServerInfo: async () => {
      const { data: serverInfo } = await invokeCommand(
        InvokeEvent.GET_ZEROTIER_SERVER_INFO,
      )
